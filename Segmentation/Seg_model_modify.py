...
class SegModel(object):
        ...
        def ResidualNet(self,nfilter,s):
            def Res_unit(x):
                ...
                x = Conv2D(b_filter,(1,1),strides=(s,s),padding='same')(x)
                x = BatchNormalization(axis=-1)(x)
                x = self.relu(x)
                x = Conv2D(b_filter,(3,3),padding='same')(x)
                x = BatchNormalization(axis=-1)(x)
                x = self.relu(x)
                x = Conv2D(nfilter,(1,1),padding='same')(x)

                ident_map = Conv2D(nfilter,(1,1),strides=(s,s),padding='same')(ident_map)
        ...
        ## Improved ResidualNet
        #--------------------encoder---------    
            inp = Input(shape=(self.input_size))
            i = inp
            i = Conv2D(16,3,padding='same')(i)           
            i = BatchNormalization()(i)
            i = ReLU()(i)
            i = bottleneck(i, 16, 32, (2, 2), shortcut=False, zero_pad=True)
            ...
            #i = bottleneck(i, 64, 128, (2, 2), shortcut=False, zero_pad=True)
            #i = bottleneck(i, 128, 128, (1, 1), shortcut=True)
            #i = bottleneck(i, 128, 128, (1, 1), shortcut=True)
            ...
            # inputNode, i = MobileNetV2(self.input_size)
